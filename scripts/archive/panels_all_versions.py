...
workbook = xlsxwriter.Workbook('all_panel_versions_' + todays_date + '.xlsx')
    ...
    while version_num <= curr_version_num:
        ...
        try:
            ...
            for gene in gene_info:
                ...
                # write to Excel file to capture all version numbers
...
# this loop does not cature incidences where the version number is in format of greater than 2 d.p, e.g. x.xxx.
# it also does not capture data from version numbers 2 d.p. long that end in 0 e.g. v1.10 (which is different to v1.1!)

    # print a mesage to the user to gauge how far through the script is.
