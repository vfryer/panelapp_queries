...
def promoted_panels():
    ...
    df1 = pd.read_sql_query("SELECT DISTINCT panel_name, panel_id, version_num FROM panelapp_info WHERE Datestamp LIKE '2017-06-30%' AND version_num <1", conn)
...
def new_genes():
    # any genes that have been added to any panels
    df1 = pd.read_sql_query("SELECT panel_name, panel_id, gene_symbol, gene_status, version_num FROM panelapp_info WHERE Datestamp LIKE '2017-06-30%' AND gene_status != 'Green'", conn)
    df2 = pd.read_sql_query("SELECT panel_name, panel_id, gene_symbol, gene_status, version_num FROM panelapp_info WHERE Datestamp LIKE '2017-07-10%' AND gene_status = 'Green' AND version_num >=1", conn)
    print("New genes: ")
    new_genes_df = pd.merge(df1, df2, on=['Panel_ID','Gene_Symbol'], how='right',indicator=True)
    print(new_genes_df)
    new_genes_df =  new_genes_df[new_genes_df['_merge'] == 'right_only']
    #new_genes_df = new_genes_df.drop('Panel_Name_y',1)
    #new_genes_df = new_genes_df.rename(columns={'Version_Num_x':'Prev_Version_Num','Panel_Name_x':'Panel_Name','Version_Num_y':'Curr_Version_Num','Gene_Status_x':'Prev_Gene_Status','Gene_Status_y':'Curr_Gene_Status'})
    new_genes_df.to_excel(writer, 'New Genes',index=False)
    print(new_genes_df)


def del_genes():
    # any genes that have been removed from any panels
    df1 = pd.read_sql_query("SELECT panel_name, panel_id, gene_symbol, gene_status, version_num FROM panelapp_info WHERE Datestamp LIKE '2017-06-30%' AND gene_status != 'Green'", conn)
    df2 = pd.read_sql_query("SELECT panel_name, panel_id, gene_symbol, gene_status, version_num FROM panelapp_info WHERE Datestamp LIKE '2017-07-10%' AND gene_status = 'Green' AND version_num >=1", conn)
    print("Removed genes: ")
    del_genes_df = pd.merge(df1, df2, on=['Panel_ID','Gene_Symbol'], how='left',indicator=True)
    print(del_genes_df)
    del_genes_df =  del_genes_df[del_genes_df['_merge'] == 'left_only']
    #del_genes_df = del_genes_df.drop('Panel_Name_y',1)
    #del_genes_df = del_genes_df.rename(columns={'Version_Num_x':'Prev_Version_Num','Panel_Name_x':'Panel_Name','Version_Num_y':'Curr_Version_Num','Gene_Status_x':'Prev_Gene_Status','Gene_Status_y':'Curr_Gene_Status'})
    del_genes_df.to_excel(writer, 'Removed Genes',index=False)
    print(del_genes_df)

def promoted_genes():
    df1 = pd.read_sql_query("SELECT DISTINCT panel_name, panel_id, gene_symbol, gene_status, version_num FROM panelapp_info WHERE Datestamp LIKE '2017-06-30%' AND gene_status != 'Green'", conn)
...
def demoted_genes():
    ...
    df1 = pd.read_sql_query("SELECT DISTINCT panel_name, panel_id, gene_symbol, gene_status, version_num FROM panelapp_info WHERE Datestamp LIKE '2017-06-30%' AND gene_status = 'Green' AND version_num >=1", conn)
...
def moi_change():
    ...
    df1 = pd.read_sql_query("SELECT DISTINCT panel_name, panel_id, gene_symbol, moi, version_num FROM panelapp_info WHERE Datestamp LIKE '2017-06-30%' AND version_num >=1", conn)
