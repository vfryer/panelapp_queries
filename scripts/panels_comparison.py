...
def new_v1_panels():
    ...
    cur.execute("SELECT DISTINCT panel_name, panel_id, version_num FROM panelapp_info WHERE Datestamp LIKE ? AND panel_name NOT IN(SELECT panel_name FROM panelapp_info WHERE Datestamp LIKE ?)",(curr_version+'%',prev_version+'%'))
    data = cur.fetchall()
    print("New v1 panels: ")
    for row in data:
        version_num = float(row[2])
        # add to tab in spreadsheet
        if version_num >= 1.0:
            print(row)
...
def del_panels():
    ...
    for row in data:
        # add tab to Excel spreadsheet containing these results 'Retired panels'
        print(row)
